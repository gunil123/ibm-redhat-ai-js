...
- firstChild : 첫번째 자손 노드 탐색 속성
- lastChild : 마지막 자손 노드 탐색 속성
- firstElementChild : (공백뺀) 첫번째 자손 노드 탐색 속성
- lastElementChild : (공백뺀) 마지막 자손 노드 탐색 속성
- nextElementSibling : (공백뺀) 다음 형제 노드 탐색 속성
- prevElementSibling : (공백뺀) 이전 형제 노드 탐색 속성
...
- 폼 요소 관련 속성과 메서드
checked	input 태그의 checkbox, radio 체크 여부(체크되면 true, 해제되면 false)
...
### 인라인박스 <label, a, span, strong, textarea, input...>
...
- 사용자 정의 속성으로 data- 로 시작한다.
- data-속성성격 = “값” 
- ex) data-order, data-study
...
- history : 윈도우에 로드한 URL 리스트의 히스토리 관리
...
- 위치 관련 메소드
- moveBy(x,y) :  창의 위치를 상대적으로 이동 
- moveTo(x,y) : 창의 위치를 절대적으로 이동
- 크기 관련 메소드
- resizeBy(x,y) : 창의 크기를 상대적으로 변경
- resizeTo(x,y) : 창의 크기를 절대적으로 변경

- window 객체의 속성과 메소드
- 대표적인 href 속성 : 완전한 형태의 url 주소 표시
### 1. 2 location 객체
...
event : 사용자가 명령할 때 작동하게 하는 시점

1. 고전 이벤트 방식 : 문서객체.on이벤트타입명 = 이벤트핸들러; 

```jsx
{문서객체.on이벤트타입명 = 이벤트핸들러;}
...
- parentNode : 부모 노드 탐색 속성
- childNodes[n] : 자손 노드 탐색 속성
...
| 메소드 이름	| 설명 | 
|--------|----------|
|`createElement(’태그명’)`|태그 요소를 생성하는 메소드|
|`appendChild()`|	생성한 태그 요소를 자식 요소로 삽입하는 메소드|
|` innerHTML=’텍스트’`	|텍스트를 추가하는 메소드|
